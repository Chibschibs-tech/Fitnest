  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(`Found ${usersResult.length} users in fallback query`)
        ...
        // Calculate revenue from customer data
        ...
        try {
          customers = await sql`
            ...
            ORDER BY created_at DESC
          `
          console.log(`Last resort: Found ${customers.length} customers without order data`)
        } catch (lastResortError) {
          console.log("Even last resort failed:", lastResortError)
          customers = []
        }
      }
    }
    ...
    console.log(`Returning ${customers.length} customers, ${activeCustomers} active`)
